...
// no difference to a Buff, and must make one to a Defensive. The weights are
// local to this test, so a literal WOULD be stable -- but it would pin the
// arithmetic of the arm rather than the rule, and break on any later change to
// how the baseline is composed. The rule is the invariant worth pinning.
//
